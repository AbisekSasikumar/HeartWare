...
# 10 sensor keys, including ECG
SENSOR_KEYS = ['Heart Rate', 'SpO2', 'Temp (\u00b0C)', 'Pressure (hPa)', 'Altitude (m)', 'Acc X', 'Acc Y', 'Acc Z', 'Air Quality', 'ECG']
...
class SensorGUI:
    ...
    def create_widgets(self):
        ...
        self.altitude_box = tb.LabelFrame(self.env_frame, text="Altitude (m)", bootstyle="primary")
        self.altitude_box.pack(side='left', expand=True, fill='both', padx=5)
        self.altitude_label = tb.Label(self.altitude_box, text="--", font=("Segoe UI", 14))
        self.altitude_label.pack(padx=10, pady=10)

        self.pressure_box = tb.LabelFrame(self.env_frame, text="Pressure (hPa)", bootstyle="primary")
        self.pressure_box.pack(side='left', expand=True, fill='both', padx=5)
        self.pressure_label = tb.Label(self.pressure_box, text="--", font=("Segoe UI", 14))
    ...
    def update_loop(self):
        ...
        if data:
            ...
            if filtered:
                ...
                self.altitude_label.configure(text=f"{latest[4]:.1f}")
                self.pressure_label.configure(text=f"{latest[3]:.1f}")
    ...
    def check_alerts(self, latest_data):
        ...
        if hr < 50 or hr > 120:
            alerts.append(f"⚠ Abnormal Heart Rate: {hr:.1f} bpm")
        if spo2 < 90:
            alerts.append(f"⚠ Low SpO₂: {spo2:.1f}%")
        if temp > 38.0:
